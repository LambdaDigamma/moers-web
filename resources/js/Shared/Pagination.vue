<template>
    ...
    <div class="flex flex-wrap">
        <template v-for="(link, key) in links">
            ...
            <span v-if="link.url === null" :key="key" class="inline-flex rounded-md shadow-sm" :class="{ 'ml-auto': link.label === 'Nächste', 'mr-3': link.label !== 'Nächste' }">
                <button disabled class="inline-flex items-center px-4 py-2 text-base font-medium leading-6 text-gray-700 transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md opacity-50">
                    {{ link.label }}
                </button>
            </span>
