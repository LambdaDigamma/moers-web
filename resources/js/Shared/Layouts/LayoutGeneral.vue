<template>
    ...
    <div>
        ...
        <div class="flex h-screen overflow-hidden bg-gray-100" @keydown.esc="hideSidebar" tabindex="0">
            ...
            <div class="md:hidden">
                ...
                <div class="fixed inset-y-0 left-0 z-40 flex flex-col w-full max-w-xs duration-300 ease-in-out transform bg-gray-800" :class="{'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen}">
                    ...
                    <div class="flex items-center flex-shrink-0 h-16 px-4 bg-gray-900">
                        <inertia-link :href="route('landingPage')" class="text-lg font-bold text-white">Mein Moers</inertia-link>
                    </div>
                    <div class="flex-1 h-0 overflow-y-auto">
                        <nav class="px-2 py-4">
                            <MenuGeneralMobile :url="url()" @navigated="hideSidebar" />
                        </nav>
                    </div>
                </div>
            </div>
...
<!--                            </div>-->
                        ...
                        <div class="flex items-center ml-4 md:ml-6" v-if="$page.props.auth.user">
                            <inertia-link :href="route('notifications')" class="relative p-1 text-gray-400 rounded-full hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring focus:text-gray-500">
                                ...
                                <span v-if="this.$page.props.auth.user.notifications_count !== 0" class="absolute bottom-0 right-0 block mb-1 mr-1 transform translate-x-1/2 translate-y-1/2 border-2 border-white rounded-full">
                ...
                <div class="fixed inset-x-0 bottom-0 pb-2 sm:pb-5" v-if="showBanner">
                        ...
                        <div class="p-2 bg-red-600 rounded-lg shadow-lg sm:p-3">
                            <div class="flex flex-wrap items-center justify-between">
                                ...
                                <div class="flex-shrink-0 order-2 sm:order-3 sm:ml-2">
                                    <button type="button" class="flex p-2 -mr-1 transition duration-150 ease-in-out rounded-md hover:bg-red-500 focus:outline-none focus:bg-red-500" @click="hideBanner">
                                        <svg class="w-6 h-6 text-white" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                        </svg>
                                    </button>
...
<script>
    ...
    import MenuGeneralMobile from "./MenuGeneralMobile";
    import MenuGeneralDesktop from "./MenuGeneralDesktop";
    ...
    export default {
        ...
        components: {
            MenuGeneralDesktop,
            MenuGeneralMobile,
            MenuItemMobile, MenuItemDesktop, MainMenuGeneral, Dropdown, FlashMessages, CookieLaw},
        data() {
            return {
                sidebarOpen: false,
                open: false,
        ...
        mounted() {
            ...
            if (this.$page.props.auth.user !== null) {
                ...
                Echo.private('App.User.' + this.$page.props.auth.user.id)
